 ...
 $TmpRoot     = "$env:USERPROFILE\.gemini\tmp"
 $LogPath     = $null

 if (Test-Path $TmpRoot) {
     foreach ($Dir in Get-ChildItem -Path $TmpRoot -Directory) {
         $RootFile = Join-Path $Dir.FullName ".project_root"
         if (Test-Path $RootFile) {
             $RootContent = (Get-Content $RootFile -Raw).Trim()
             if ($RootContent -eq $CurrentPath) {
                 $LogPath = Join-Path $Dir.FullName "chats"
                 break
             }
         }
     }
 ...
 if ($null -eq $LogPath -or -not (Test-Path $LogPath)) {
     Write-Host "ERRO: Nenhuma pasta de logs Gemini encontrada para $CurrentPath"
     return
 }
