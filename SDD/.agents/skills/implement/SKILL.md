---
name: implement
description: Executes implementation tasks with precision following TechSpec and project guidelines, with automatic TDD decision per task type. Use to implement a specific task from the task document, producing traceable code ready for code review.
---
...
**Execute os testes via script:**
```
pwsh .agents/skills/implement/scripts/run-tests.ps1
```
O script detecta o runner automaticamente, salva o log completo em `.test-logs/` e retorna apenas o sumário.
- ✅ Todos passaram → log apagado automaticamente, prossiga.
- ❌ Falhas → log mantido em `.test-logs/test-run-<timestamp>.log`. Use a ferramenta `Read` no arquivo indicado para inspecionar as falhas, corrija e re-execute o script.

Após os testes passarem, percorra **cada critério de aceite** da task:

| Critério | Status | Observação |
|----------|--------|------------|
| [AC 1 — Dado X, quando Y, então Z] | ✅ / ❌ / ⚠️ | [evidência ou o que falta] |
| [AC 2] | ✅ / ❌ / ⚠️ | |
| Testes escritos e passando | ✅ / ❌ / ⚠️ | [sumário do script] |
...
## FASE 4.5 — Code Review

Apresente ao usuário:

> "Task concluída. Deseja submeter os arquivos a um code review independente antes de prosseguir?
> **(a) Sim** — QA sub-agente revisa em contexto fresco (recomendado para tasks grandes)
> **(b) Inline** — review rápido no contexto atual
> **(c) Não** — prosseguir direto para o relatório"

**Se (a) — Sub-agente:**
Invoque o agente QA usando o mecanismo nativo do seu ambiente, passando:
- Lista dos arquivos criados/modificados nesta task
- Referência à task e seus critérios de aceite
- Instrução para aplicar as dimensões do skill `/code-review`
- Claude Code: ferramenta `Task` + `.claude/agents/qa.md`
- Antigravity: `invoke_subagent` com agente `qa`
- Codex / outros: mecanismo nativo de sub-agentes

Receba o relatório do QA e apresente ao usuário. Se houver findings 🟡 ou 🔴, aguarde decisão antes de avançar:
```
(i) Corrigir agora, inline nesta task
(ii) Criar task de bug-fix separada
(iii) Criar v2 desta task incorporando as correções
(iv) Ignorar e concluir mesmo assim
```

**Se (b) — Inline:**
Execute o review no contexto atual aplicando estas dimensões:
...
Findings 🔵: registre no relatório final e prossiga. Findings 🟡 ou 🔴: apresente e aguarde decisão (mesmas opções acima).

**Se (c):** prossiga diretamente para FASE 5.
