---
name: tasks
description: Transforms PRD and TechSpec into complete, self-contained implementation tasks ordered by dependency, with parallelism opportunities identified. Use after approving PRD and TechSpec to generate an executable work plan ready for developers or AI agents.
---
...
2. Após concluir cada Épico completo (todas as suas User Stories e Tasks), adicione-o ao arquivo com `append` antes de iniciar o próximo Épico.
3. Se o contexto esgotar durante a geração, o conteúdo já salvo não se perde.
A FASE 6 apenas atualiza `memory/state.md` — não re-salva o documento principal.
...
1. Valide o documento gerado:
   ```
   pwsh .agents/skills/tasks/scripts/validate.ps1 -File docs/tasks/[nome-kebab-case]-tasks.md
   ```
   Se retornar ❌, corrija antes de prosseguir.
2. Atualize `memory/state.md` — seção **Tasks**:
...
2. **Se o usuário disser "Sim":**
   - Invoque os sub-agentes especializados usando o mecanismo nativo do seu ambiente, instruindo-os a **ler os arquivos recém-salvos** em `docs/tasks/`.
   ...
   - *Se o ambiente não suportar sub-agentes:* Simule as personas de Qualidade e Arquitetura em auto-reflexão no próprio chat.
