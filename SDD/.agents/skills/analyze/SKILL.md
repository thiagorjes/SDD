---
name: analyze
description: Performs cross-artifact analysis between PRD, TechSpec and Tasks in the SDD process, detecting inconsistencies, coverage gaps, ambiguities, and contradictions. Use when validating artifact consistency or reviewing specifications before starting implementation.
---
...
**Verifique rastreabilidade de IDs antes de ler os artefatos:**
```
pwsh .agents/skills/analyze/scripts/trace-ids.ps1 `
  -PrdFile docs/prd/[nome]-prd.md `
  -TechspecFile docs/techspec/[nome]-techspec.md `
  -TasksFile docs/tasks/[nome]-tasks.md
```
O script retorna IDs do PRD sem cobertura no TechSpec ou Tasks. Use o resultado como entrada para os passes de detecção — não precisa re-derivar manualmente.
