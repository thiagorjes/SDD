...
2. Após concluir cada seção numerada, adicione-a ao arquivo com `append` antes de iniciar a próxima.
3. Nunca exiba o documento completo no chat — apenas informe o caminho e um resumo ao final.
...
2. **Se o usuário disser "Sim":**
   - Invoque os sub-agentes especializados usando o mecanismo nativo do seu ambiente, instruindo-os a **ler os arquivos recém-salvos** e avaliar gargalos de performance, falhas de modelagem, segurança, riscos operacionais e testabilidade dos contratos.
   ...
   - *Se o ambiente não suportar sub-agentes:* Simule as personas de Arquitetura, Segurança, Database, DevOps e Qualidade em auto-reflexão no próprio chat.
